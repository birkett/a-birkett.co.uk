...
suite('Count files in the dist folder', () => {
    ...
    describe('JS', () => {
        it('Should have 1 JS file', () => {
            testFilesPresent(BuildConstants.outputDirectory, '.js', 16, 1);
        });
    });

    describe('Favicons', () => {
        it('Should have 10 PNG favicons', () => {
            testFilesPresent(BuildConstants.outputDirectory, '.png', 16, 10);
        });

        it('Should have 1 SVG favicon', () => {
            testFilesPresent(BuildConstants.outputDirectory, '.svg', 16, 1);
        });

        it('Should have 1 ICO favicon', () => {
            testFilesPresent(BuildConstants.outputDirectory, '.ico', 16, 1);
        });
    });
});
