...
echo '<channel>';
echo '<atom:link href="' . GetBaseURL() . 'feed" rel="self" type="application/rss+xml" />';
...
echo '<copyright>Copyright (C) 2007-' . date('Y') . ' Anthony Birkett - ' . GetBaseURL() . '</copyright>';
...
while (list($id, $timestamp, $title, $content, $draft) = GetDatabase()->GetRow($result)) {
    ...
    echo '<link>' . GetBaseURL() . "blog/" . $id . '</link>';
